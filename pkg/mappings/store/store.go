...
func NewStore(backend Backend) synccontext.MappingsStore {
	return &Store{
		backend:                   backend,
...
func (s *Store) dumpMappings(ctx context.Context) {
	...
	for mapping := range s.mappings {
		klog.FromContext(ctx).Info("Mapping", "host", mapping.Host().String(), "virtual", mapping.Virtual().String())
	}
}
...
func (s *Store) RecordReference(ctx context.Context, nameMapping synccontext.NameMapping) error {
...
func (s *Store) RecordLabel(ctx context.Context, labelMapping synccontext.LabelMapping) error {
	// check if we have the owning object in the context
	belongsTo, ok := synccontext.MappingFrom(ctx)
	if !ok {
		return nil
...
func (s *Store) RecordLabelCluster(ctx context.Context, labelMapping synccontext.LabelMapping) error {
	// check if we have the owning object in the context
	belongsTo, ok := synccontext.MappingFrom(ctx)
	if !ok {
		return nil
