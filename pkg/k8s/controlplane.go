...
import (
	...
	"github.com/loft-sh/vcluster/pkg/k8s/node"
...
func StartK8S(
	...
	if apiServer.Enabled {
		eg.Go(func() error {
			...
			if len(apiServer.Command) > 0 {
				...
				args = append(args, binaryDir+"/kube-apiserver")
				args = append(args, "--advertise-address=192.168.194.211")
	...
	if controllerManager.Enabled {
		eg.Go(func() error {
			...
			if len(controllerManager.Command) > 0 {
				...
				args = append(args, binaryDir+"/kube-controller-manager")
				args = append(args, "--service-cluster-ip-range="+serviceCIDR)
	...
	if vConfig.ControlPlane.Advanced.VirtualScheduler.Enabled || true {
		eg.Go(func() error {
			// build flags
			args := []string{}
			if len(scheduler.Command) > 0 {
				...
				args = append(args, binaryDir+"/kube-scheduler")
